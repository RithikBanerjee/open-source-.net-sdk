...
namespace ConsoleApplication1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if(demo.AuthenticateRequest())
                demo.GetInvoices();
            //var response = fetchData.save();
            Console.ReadLine();
        }
    }
    public class DemoClass
    {
        ...
        private Request PrepareRequest()
        {
            ...
            return new Request()
            {
                ...
                Keys = new CipherKeys()
                {
                    GSTNAppKey = "124HJs0927Yhst682JSB654FSYIJB733"
                }
            };
        }
        public bool AuthenticateRequest()
        {
            ...
            Console.WriteLine("\nAuth Token Generated?(0/1)");
            if(!Int64.TryParse(Console.ReadLine(), out var generateOtp))
            {
                Console.WriteLine("\nInvalid Input.");
                return false;
            }

            if (Equals(generateOtp, 1))
            {
                Console.WriteLine("\nAuth Token:");
            ...
            if(!response.Status)
            {
                Console.WriteLine($"{response.OutcomeTransaction} Try Again.");
                return false;
            }
            ...
            {
                Console.WriteLine($"Error: {authResponse.OutcomeTransaction} Try Again.");
                return false;
            }
        ...
        {
            //myCreds.GstnCredentials.GSTN = "27GSPMH1331G1ZW";
            while (true)
            {
                Console.WriteLine("\nReturn Type:");
                ...
                Console.WriteLine("\nFinancial Year:");
                var fp = Console.ReadLine();
                var data = GetRequest(returnType, secName, fp);
                Console.WriteLine("\nResponse:\n" + data);
            }
        }

        
        private string GetRequest(string returnType, string secName, string fp)
        {
            MyRequest.Header.ReturnPeriod = fp;
            ...
            switch (returnType)
            {
                case "gstr1":
                    ...
                    {
                        case "b2b":
                            response = GSTR1Service.GetB2BInvoices(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                        case "cdnr":
                            response = GSTR1Service.GetCDNRInvoices(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                        case "b2cs":
                            response = GSTR1Service.GetB2CSInvoices(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                        case "hsn":
                            response = GSTR1Service.GetHSNSummary(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                    }
                    break;
                case "gstr2a":
                    ...
                    {
                        case "b2b":
                            response = GSTR2AService.GetB2BInvoices(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                        case "cdn":
                            response = GSTR2AService.GetCDNInvoices(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                    }
                    break;
                case "gstr3b":
                    response = GSTR3BService.GetDetails(MyRequest, fp, MyRequest.Header.GSTN);
                    break;
                case "itc":
                    switch (secName)
                    {
                        case "sum":
                            response = ITC04Service.GetSummary(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                        case "inv":
                            response = ITC04Service.GetInvoices(MyRequest, fp, MyRequest.Header.GSTN);
                            break;
                    }
                    break;
            }
